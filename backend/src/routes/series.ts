...
router.post("/:id/pick", (req: Request, res: Response) => {
  ...
  const series: number = parseInt(req.params.id);
  const sql: string = "CALL pickSeries(?, ?);";
  db.query(sql, [series, userId], (err: MysqlError | null) => {
    if (err) {
      console.log(err);
      res.sendStatus(500);
    }
...
router.post("/:id/unpick", (req: Request, res: Response) => {
  const userId: number = req.body.userId;
  const series: number = parseInt(req.params.id);
  const sql: string = "CALL unPickSeries(?, ?);";
  db.query(sql, [series, userId], (err: MysqlError | null) => {
    if (err) {
      console.log(err);
      res.sendStatus(500);
...
router.post("/episodes/:id/seen", (req: Request, res: Response) => {
  const userId: number = req.body.userId;
  const seen: boolean = req.body.seen;
  const episode: number = parseInt(req.params.id);
  const sql: string = "CALL seenEpisode(?, ?, ?);";
  db.query(sql, [episode, userId, seen], (err: MysqlError | null) => {
    if (err) {
      console.log(err);
      res.sendStatus(500);
    }
...
// Get series helper
function getMySeries(id: number): Promise<Series[]> {
  return new Promise((resolve, reject) => {
    const seriesSql: string = "CALL getAllMySeries(?)";
    db.query(seriesSql, [id], (err: MysqlError | null, seriesResult: any) => {
      if (err) {
        reject(err);
      }
      const series: Series[] = seriesResult[0];
