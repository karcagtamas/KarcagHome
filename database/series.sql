...
/* Add series */
CREATE OR REPLACE PROCEDURE addSeries
(_name VARCHAR
(100), _creater INT
(11))
BEGIN
    INSERT INTO series
        (name, creater, lastModifier)
...
/* Modify series */
CREATE OR REPLACE PROCEDURE updateSeries
(_id INT
(11), _name VARCHAR
(100), _updater INT
(11))
...
/* Delete series */
CREATE OR REPLACE PROCEDURE deleteSeries
(_id INT
(11))
BEGIN
...
CREATE OR REPLACE PROCEDURE getSeasons
(_series INT
(11))
BEGIN
    SELECT
...
/* Get season by id */
CREATE OR REPLACE PROCEDURE getSeason
(_id INT
(11))
BEGIN
    SELECT
...
CREATE OR REPLACE PROCEDURE addSeason
(_series INT
(11), _number INT
(2), _episodes INT
(3))
...
/* Delete season */
CREATE OR REPLACE PROCEDURE deleteSeason
(_id INT
(11))
BEGIN
...
CREATE OR REPLACE PROCEDURE getEpisodes
(_season INT
(11))
BEGIN
    SELECT
...
/* Get episode by id */
CREATE OR REPLACE PROCEDURE getEpisode
(_id INT
(11))
BEGIN
    SELECT
...
/* Delete episode */
CREATE OR REPLACE PROCEDURE deleteEpisode
(_id INT
(11))
BEGIN
    DECLARE _season INT
...
BEGIN
    ...
    FROM series
        INNER JOIN users AS creater ON creater.id = series.creater
        INNER JOIN users AS modifier ON modifier.id = series.lastModifier
        INNER JOIN switch_series_users ON switch_series_users.series = series.id
    WHERE switch_series_users.user = _id
    ORDER BY series.name;
END;

CREATE OR REPLACE PROCEDURE pickSeries(_series INT(11), _user INT(11))
    BEGIN
        INSERT INTO switch_series_users(series, user)
        VALUES(_series, _user);
        /* TODO INSERT INTO episode switch */
    END;

CREATE OR REPLACE PROCEDURE unPickSeries(_series INT(11), _user INT(11))
    BEGIN
        DELETE FROM switch_series_users WHERE series = _series AND user = _user;
        DELETE FROM switch_episodes_users WHERE series = _series AND user = _user;
    END;

CREATE OR REPLACE PROCEDURE seenEpisode(_episode INT(11), _user INT(11), _value BOOLEAN)
    BEGIN
        UPDATE switch_episodes_users SET seen = _value WHERE episode = _episode AND user = _user;
    END;
