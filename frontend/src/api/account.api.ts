...
export const accountApi = {
  ...
  update: (id: number, data: AccountEditDTO) =>
    api.put<AccountDTO>(`${ACCOUNT_API}/${id}`, data).then((res) => res.data),
  delete: (id: number) => api.delete(`${ACCOUNT_API}/${id}`),
  getSummary: (id: number) => api.get<AccountSummaryDTO>(`${ACCOUNT_API}/${id}/summary`).then((res) => res.data),
};
